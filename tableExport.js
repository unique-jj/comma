...
(function(root, factory) {
	...
	var TableExport = function(selectors, options) {
		...
		if (!self) return new TableExport(selectors.el.querySelector('.tui-grid-rside-area'), options);	// @author JELLY
		// if (!self) return new TableExport(selectors, options);	// origin
		...
		self.selectors.forEach(function(el) {
			...
			context.unMerge = settings.unMerge ;
	...
	TableExport.prototype = {
		...
		defaults: {
			...
			unMerge: false	//셀이 병합된 상태로 다운로드 된다. true  경우 셀 병합이 해제된 상태로 다운로드 된다. (default: false)
		},
		/**
		 * Constants
		 * @memberof TableExport.prototype
		...
		exporters: {
			build: function(context, key) {
				...
				var totalMergeRow = new Array();		// 셀 병합 해제를 했을 경우 dataURI 에  추가되야하는 데이터
				...
				.map(function(val, ir) {
					...
					.map(function(val, ic) {
						...
						// 병합해제 다운로드의 경우 실행	@author JELLY
						if(settings.unMerge){
							// merge date를 체크하여 담아준다.
							if(val.hasAttribute("rowspan")){
								if(val.rowSpan > 1){
									// irStart = 시작 row / irEnd = 반복 횟수 / icStart = Cell 순번 / value = 값
									totalMergeRow.push({irStart: ir+1, irEnd : ir + val.rowSpan - 1, icStart : ic, value: settings.formatValue(val.textContent)});
								}
							}
						}
				...
				if(settings.unMerge){
					// 병합된 셀이 있다면 값을 추가해준다.
					if(totalMergeRow.length > 0){
						for(var i = 0; i <totalMergeRow.length; i++){
							var tempData = totalMergeRow[i];
							var addData = {
								v: tempData.value,
								t: ""
							};
							for(var countNum = tempData.irStart; countNum<= tempData.irEnd; countNum++){
								dataURI[countNum].splice(tempData.icStart, 0, addData);
							}
						}
					}
				}
		...
		createObjButton: function(hashKey, dataObject, myContent, myClass, bootstrapSettings) {
			...
			exportButton.setAttribute("id", "hiddenExcel");		// @author JELLY
			exportButton.setAttribute("style", "display:none");	// @author JELLY
